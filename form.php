...
<div class="container">
    ...
    <h6 id="form_dec">Please enter a name and a email address and a valid file</h6>
        <form class="form-group" method="POST" action="" enctype="multipart/form-data">
            ...
            <input class="form-control<?=$errors["userName"]?' red':""?>" type="text" name="userName" placeholder="name" value="<?=$user_name?>" />
            <?=$errors["userName"]?'<span class="emptyFiledsAlerts">'.$errors["userName"].'</span>':""?>

            <input id="Pass" class="form-control<?=$errors["email"]?' red':""?>" type="email" name="email" placeholder="email" value="<?=$email?>"/>
